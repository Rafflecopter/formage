...
function path_to_field(path, prefix, path_name, tree) {
    ...
    if (fieldOptions.validate) {
        if (fieldOptions.validate.length === 2 && _.isFunction(fieldOptions.validate[0]) && _.isString(fieldOptions.validate[1])) {
            validators.push(function (value) {
                var result = fieldOptions.validate[0](value);
                return result ? true : fieldOptions.validate[1];
            });
        } else {
            validators.push(function myValidate(value) {
                var validate = _.clone(fieldOptions.validate),
                    current = null,
                    result = true;
                ...
                function next(res) {
                    result = result === true ? res && true || current.msg : result;
                    if (result === true && validate.length > 0) {
                        current = validate.shift();
                        current.validator(value, next)
                    }
                }
            });
        }
