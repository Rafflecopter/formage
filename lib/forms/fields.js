...
var BaseField = exports.BaseField = Class.extend({
    ...
    unbind: function () {
        this.value = (function(x,d){return x===undefined ? d : x})(this.data[this.hirarchy_name], this.value);
    ...
    validate: function () {
        var p = new Promise();
        if (this.value === null || this.value === [] || this.value === undefined || this.value === '') {
            if (this.required) {
...
var FilepickerField = exports.FilepickerField = BaseField.extend({
    init: function (options) {
        ...
        if (FILEPICKER_API_KEY && AVIARY_API_KEY) {
            this.header_lines = [
                '<script>var FILEPICKER_API_KEY="' + FILEPICKER_API_KEY + '", AVIARY_API_KEY="' + AVIARY_API_KEY + '";</script>',
